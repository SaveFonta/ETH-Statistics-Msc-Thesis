...
standard fixed design and extending to \textit{Bayesian group sequential} and \textit{dual criterion}
designs.
The main objectives are listed below.

\begin{itemize}
    \item Starting from the average T1E rate of \citet{best_beyond_2025}, defined for borrowing on the
    control arm, develop an analogous \textit{average power} by integrating the power conditional on
    the control parameter over the same design prior. Derive closed form expressions and properties of
    both metrics under normal priors, together with the sample size computation they imply, extending
    the derivations to \textit{dual criterion} designs.

    \item Apply these metrics to a Crohn's disease PoC trial. Then, extend the application
    using a robust MAP analysis prior (i.e.\ a mixture of normals), whose operating characteristics are obtained by numerical
    integration rather than in closed form, and the sample size computation is extended to these
    normal mixture priors accordingly. Quantify how much \textit{drift} a given robustness weight
    tolerates by perturbing the design prior.

    \item Starting from the EUII definition from \citep{held2025balancing}, use these averaged metrics to define an average EUII, 
    extending the metric to the Bayesian borrowing framework, and characterize its asymptotic
    behavior.
    ...
    useful under prior conflict, defining the setting in which the
    ...
    a common average T1E rate and average power, following \citet{held2025balancing}.

\end{itemize}


The remainder of the thesis is organized as follows. Chapter~\ref{sec:methodology} develops the
methodology: the MAP prior and its robust extension (Section~\ref{subsec:formulating_priors}), the
average metrics with their closed forms and properties under normal priors
(Section~\ref{subsec:metrics_bayesian}), the \textit{dual criterion}
(Section~\ref{subsec:dual_criterion_methods}), Bayesian group sequential designs
(Section~\ref{subsec:sequential}), and the extension of the EUII to the borrowing framework, both for
fixed and for sequential designs (Section~\ref{subsec:euii}). Chapter~\ref{sec:Applications} applies
the framework to a PoC case study in Crohn's disease: the fixed design (Section~\ref{sec:fixed_design}),
one and two interim analyses (Sections~\ref{sec:one_interim} and~\ref{sec:two_interim}), and a
comparison of designs calibrated to common error rates across the number of looks, the boundary shape,
and futility stopping (Section~\ref{sec:calibrated_comparison}). Chapter~\ref{sec:conclusion} concludes
with a discussion of the findings, their limitations, and directions for future work.
