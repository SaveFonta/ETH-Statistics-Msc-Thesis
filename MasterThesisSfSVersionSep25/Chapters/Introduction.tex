...
ethical considerations \citep{dimasi_innovation_2016}. For instance, recruiting a sufficient number of patients, particularly in the case of 
...
characteristics, relying heavily on strict control of the type I error (T1E) rate. A fundamental limitation of this paradigm is that it treats every trial 
...
probability of being assigned to a placebo group. As noted by \citet{ohagan_assurance_2005}, \textit{``the design of a trial 
...
maximize the probability of a satisfactory outcome''}.
...
\label{subsec:bayesian_designs_intro}
...
\begin{itemize}
    ...
    demographic, such as extrapolating adult data to pediatric cohorts, the prior can be placed on the treatment contrast directly \citep{psioda_bayesian_2019}.
\end{itemize}
...
statistically significant result \citep{spiegelhalter_predictive_1986, ohagan_assurance_2005}. It is closely related to the probability of study success \citep{wang_evaluating_2013}
and to Bayesian \textit{predictive power} \citep{spiegelhalter_monitoring_1986, micheloud_power_2022}.

This thesis focuses on borrowing historical information on the control arm. In this setting, traditional assurance is not the relevant metric, since
...
the \textit{design} prior represents the plausible true underlying value of the control parameter. This reformulation is attractive because it restores the
...
not available in closed form but can be approximated by a mixture of conjugate distributions using simulation methods. However, the standard 
...
This \textit{robust MAP} prior \citep{schmidli_robust_2014} responds more rapidly to \textit{prior-data conflict}, shifting posterior weight toward the robust component as
divergence between historical and current data grows. This thesis focuses primarily on the MAP framework and its robust extension. A scoping review of Phase II trials identified 37 studies applying
Bayesian information borrowing, of which $56.8\%$ borrowed in order to supplement the control arm, and roughly a fifth of those
borrowing from external sources did so through robust mixture priors \citep{weru2026uptake}. Recent examples include
\citet{richeldi2022trial} and \citet{chen2024efficacy}.
...
The \textit{dual criterion} (DC) addresses this by requiring two conditions rather than one. Developed for proof of concept (PoC)
trials, where the decision is whether to continue development \citep{gsponer_practical_2014, fisch2015bayesian, walley2015advantages}, it
answers two distinct questions. The first asks whether the drug works at all, and demands high posterior confidence of a positive effect. The second asks whether the drug has a relevant clinical effect, and demands only moderate
...
effect that is significant but marginal may suggest reconsidering the target population.


\citet{fisch2015bayesian} further show that under this DC, an informative prior built from historical placebo
data can substitute for a large part of the trial, with a design of twelve treated and six control patients performing
comparably to one of forty patients split evenly.


\citet{roychoudhury_beyond_2018} later formalize the DC within a frequentist framework, 
in which trial success requires both statistical significance and a clinically relevant estimated effect: the effect estimate must reach a prespecified \textit{Decision Value} (DV),
...
A classical fixed design commits in advance to a single analysis at a predetermined sample size. Interim analyses relax this constraint and allow the trial to stop earlier. The motivation is both ethical and financial: stopping early for efficacy makes a working
...
on the difference between the two arms, its behavior across looks can be represented as a discretely observed Brownian 
...
\citep{pocock_group_1977}, O'Brien and Fleming's boundaries \citep{obrien_multiple_1979}, Haybittle and Peto
\citep{haybittle_repeated_1971, peto_design_1976}, Wang and Tsiatis \citep{wang_approximately_1987}, and the
\textit{alpha-spending} function \citep{lan_demets_discrete_1983}.
...
since it can only remove trials that would have failed regardless. \citet{zhu2017bayesian} propose an algorithm to calibrate the Bayesian thresholds to a target T1E rate, and \citet{shi2019control} 
...
The DOR is the natural measure of evidence here: it is the odds ratio of a positive to a negative trial outcome, so it
summarizes power and T1E rate in a single number and is large exactly when a design separates a true effect from no effect.
Taken alone it rewards any design that simply recruits more patients, which is why it is normalized: dividing the evidence on
the log scale by the sample size asks how much evidence each patient buys, and places a small efficient design and a large
one on the same scale. When the design is sequential the sample size is itself random, so the normalization is by the
expected inverse sample size on each branch rather than by a fixed count, which is what makes the index apply unchanged to
designs that can stop early.


\section{Objectives}

This thesis develops average operating characteristics for Bayesian designs that borrow historical
information on the control arm, and uses them to balance the evidentiary value a design produces
against the number of patients it requires. The framework starts from the standard fixed design
and extends to Bayesian group sequential and dual criterion (DC) designs.
The main objectives are listed below.

\begin{itemize}
    \item Take the idea already behind \textit{assurance} and \textit{predictive power}
    (Section~\ref{subsec:bayesian_designs_intro}), where power is averaged over a prior instead of being
    evaluated at a single point, and apply it to the control parameter rather than to the treatment effect.
    This gives an \textit{average power} (avgPower), the counterpart of the average T1E rate (avgT1E). Derive closed form
    expressions and properties of both metrics under normal priors, together with the sample size computation
    they imply, and generalize the derivation to DC designs.
    ...
    form. Quantify how much drift a given robustness weight tolerates by perturbing the
    design prior.

    \item Combine these averaged metrics into an average Experimental Unit Information Index
    (EUII) for the Bayesian borrowing framework, and characterize its asymptotic behavior.

    \item Extend these metrics, and the EUII built from them, from fixed designs to group sequential
    ...
    the number of interim analyses and the stopping rule, calibrating every design to
...
common error rates across the number of looks, the stopping rule, and futility stopping
...
Appendix~\ref{sec:appendix_normal_priors} derives the closed form expressions for
the average metrics under normal priors, Appendix~\ref{sec:app_asymptotic} their asymptotic behavior, and
Appendix~\ref{app:computational_framework} sets out the computational framework behind the average metrics.
