...
This thesis asks how to balance the evidentiary value a trial design produces against the number of
patients it requires, when historical information is borrowed on the control arm. The objectives are
the following.

\begin{itemize}
    \item Complete the set of average operating characteristics for control arm borrowing. This
    covers the average T1E rate of \citet{best_beyond_2025} together with an analogous average power.
    The latter replaces \textit{assurance}, which averages over uncertainty in the treatment effect
    rather than over the control parameter and is therefore not the relevant quantity when historical
    information is borrowed on the control arm. Closed form expressions and the properties of both
    metrics are derived under Normal priors, together with the sample size computation they imply,
    which for mixture analysis priors admits no closed form.

    \item Extend these metrics, and the EUII built from them, from fixed designs to group sequential
    designs with robust mixture priors, covering both a single criterion and a \textit{dual criterion}
    and allowing stopping for efficacy and for futility.

    \item Establish what the EUII does and does not measure. In particular, determine whether it can
    be read as a summary of design quality when the design prior conflicts with the analysis prior,
    and delimit the setting in which the index is informative.

    \item Use the EUII to evaluate whether the sequential framework delivers higher evidentiary value
    per patient than the fixed design framework, comparing designs calibrated to a common average T1E
    rate and average power rather than to a common threshold, following \citet{held2025balancing}.

    \item Quantify how much \textit{drift} a given robustness weight tolerates, by perturbing the
    design prior through switching, shifting and scaling, and thereby give the choice of that weight
    an empirical basis rather than a conventional one.
\end{itemize}
