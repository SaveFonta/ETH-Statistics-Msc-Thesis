...
\section{Setup}
In this derivation, we assume a common per patient variance $\sigma^2$. Thus, we can write: 
...
\[
  ...
  n_T=r_T n_C,\qquad n_{\text{trial}}=n_C(1+r_T).
\]
...
\begin{equation}\label{eq:vars}
  ...
  V_{\text{samp}} = W^2 v_C^2 + v_T^2,\qquad
  V_{\text{marg}} = V_{\text{samp}} + (1-W)^2\sigma_{D,C}^2,
\end{equation}
...
$\hat\delta \mid \delta \sim\mathcal N(\delta+\beta,\,V_{\text{marg}})$.

Thus, following from \eqref{eq:app_avgpower_closed_form} and \eqref{eq:app_avgt1e_closed_form}, we can write:
\begin{equation}\label{eq:avg}
  \avgPow(\delta)=\Phi(b),\quad b=\frac{\delta+\beta-k}{\sqrt{V_{\text{marg}}}},\qquad
  \avgTIE=\avgPow(0)=\Phi(a),\quad a=\frac{\beta-k}{\sqrt{V_{\text{marg}}}} .
\end{equation}
...
\begin{equation}\label{eq:logeuii}
  \boxed{\;\log\EUII=\frac{\Lodd(b)-\Lodd(a)}{n_{\text{trial}}}\;}.
\end{equation}
...
We now obtain the limits of the arguments $b$ and $a$ defined in Equation~\eqref{eq:avg}, analyzing their numerator and denominator separately.
...
\paragraph{Denominator} From Equation~\eqref{eq:vars}, $V_{\text{marg}} = W^2 v_C^2 + v_T^2 + (1 - W)^2 \sigma_{D,C}^2$. Both $v_C^2 = \sigma^2/n_C$ and $v_T^2 = \sigma^2/(r_T n_C)$ are $O(1/n_C)$, and since $W = n_C/(n_C+n_A) \to 1$, $W^2 v_C^2 = O(1/n_C)$. The 
...
\]
so the common denominator vanishes, $\sqrt{V_{\text{marg}}} \to 0^+$.
...
\[
  b = \frac{\delta + \beta - k}{\sqrt{V_{\text{marg}}}} \;\longrightarrow\; +\infty .
\]
...
\begin{itemize}
  ...
  \[
    a = \frac{\beta - z_p \sqrt{V_\delta}}{\sqrt{V_{\text{marg}}}}
      = \frac{\,\beta/\sqrt{V_\delta} - z_p\,}{\sqrt{V_{\text{marg}}/V_\delta}} .
  \]
  Since $\beta = O(1/n_C)$ and $\sqrt{V_\delta} = O(1/\sqrt{n_C})$, the ratio $\beta/\sqrt{V_\delta} = O(1/\sqrt{n_C}) \to 0$, and $V_\delta,\,V_{\text{marg}}$ share the same order, so $V_{\text{marg}}/V_\delta \to 1$. Hence $a \to -z_p$.
\end{itemize}
...
Recall from Equation~\eqref{eq:logeuii} that $\log\EUII = \dfrac{\Lodd(b)-\Lodd(a)}{n_{\text{trial}}}$. We can use the fact that $\Lodd(x)\approx\tfrac{x^2}{2}$. For $q>0$ we have $b\to+\infty$ and $a\to-\infty$, so, using $\Lodd(-x)=-\Lodd(x)$,
\[
  \Lodd(b)-\Lodd(a) \approx \frac{b^2}{2} - \left(-\frac{a^2}{2}\right) = \frac{b^2+a^2}{2}.
\]
Substituting $b=\dfrac{\delta+\beta-k}{\sqrt{V_{\text{marg}}}}$ and $a=\dfrac{\beta-k}{\sqrt{V_{\text{marg}}}}$ with their numerator limits $\delta+\beta-k\to\delta-q$ and $\beta-k\to-q$,
\[
  \frac{b^2+a^2}{2} \;=\; \frac{(\delta-q)^2+q^2+o(1)}{2\,V_{\text{marg}}},
\]
so that
\begin{equation}\label{eq:app_logeuii_asymp}
  \log\EUII \;=\; \frac{(\delta-q)^2+q^2+o(1)}{2\,V_{\text{marg}}\,n_{\text{trial}}} .
\end{equation}
The only quantity left to evaluate is the product $V_{\text{marg}}\,n_{\text{trial}}$.
...
\[
  V_{\text{marg}} = \sigma^2\!\left(\frac{1}{n_C}+\frac{1}{n_T}\right) + O\!\left(\frac{1}{n_C^2}\right).
\]
Hence, with $n_{\text{trial}}=n_C+n_T$ and $n_T=r_T n_C$,
\[
  V_{\text{marg}}\,n_{\text{trial}}
  \;=\; \sigma^2\!\left(\frac{1}{n_C}+\frac{1}{n_T}\right)(n_C+n_T) + O\!\left(\frac{1}{n_C}\right)
  = \sigma^2\,\frac{(n_C+n_T)^2}{n_C\, n_T} + O\!\left(\frac{1}{n_C}\right)
  = \frac{\sigma^2(1+r_T)^2}{r_T} + O\!\left(\frac{1}{n_C}\right) .
\]
...
\end{equation}
For $q=0$ the argument $a\to-z_p$ stays finite, so $\Lodd(a)/n_{\text{trial}}\to0$ and only the $b$ term contributes. The result is Equation~\eqref{eq:app_ceiling} evaluated at $q=0$.
...
\begin{equation}\label{eq:app_kdc}
  k_{\DC} = \max\!\Big(\,q + z_p\sqrt{V_\delta},\ \ \DV\,\Big).
\end{equation}

Everything else is unchanged: the DC average metrics are still $\avgPow^{\DC}(\delta) = \Phi(b_{\DC})$ and $\avgTIE^{\DC} = \Phi(a_{\DC})$, with
\[
  b_{\DC} = \frac{\delta+\beta-k_{\DC}}{\sqrt{V_{\text{marg}}}},
  \qquad
  a_{\DC} = \frac{\beta-k_{\DC}}{\sqrt{V_{\text{marg}}}},
\]
and $\log\EUII^{\DC} = \big(\Lodd(b_{\DC})-\Lodd(a_{\DC})\big)/n_{\text{trial}}$ exactly as in Equation~\eqref{eq:logeuii}.

\paragraph{Threshold at the limit}
The derivation for the SC used the threshold only through its limit $k\to q$, so
\[
  k_{\DC} \;\longrightarrow\; \max(q,\DV) = \DV,
\]
since $\DV > q$. We can thus repeat the computations of the SC case, replacing $q$ with $\DV$: the numerators become $\delta+\beta-k_{\DC}\to\delta-\DV$ and $\beta-k_{\DC}\to-\DV$, and for $\delta>\DV$ we have $b_{\DC}\to+\infty$ and $a_{\DC}\to-\infty$.


\paragraph{Final asymptotic EUII}
Substituting $q$ with $\DV$ into Equation~\eqref{eq:app_ceiling} yields:
\begin{equation}\label{eq:app_ceiling_dc}
  \lim_{n_C\to\infty}\EUII^{\DC}(n_C)
